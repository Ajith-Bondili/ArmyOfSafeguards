...
def aggregate_results(results: Dict[str, Dict[str, Any]], threshold: float = 0.7) -> Dict[str, Any]:
    ...
    for safeguard_name, result in results.items():
        ...
        if safeguard_name == 'factuality':
            ...
            is_problematic = (confidence >= threshold and label != 'LABEL_0')
...
if __name__ == "__main__":
    ...
    if result['flags']:
        ...
        for flag in result['flags']:
            print(f"  - {flag['safeguard']}: {flag['label']} (confidence: {flag['confidence']:.2%})")
    
    print("\nIndividual Results:")
    for safeguard, res in result['individual_results'].items():
        if 'error' not in res:
            print(f"  {safeguard}: {res.get('label')} (confidence: {res.get('confidence', 0):.2%})")
        else:
